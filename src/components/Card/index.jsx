...
// create cards-------------
async function createCard(listId,cards,setCards,cardTitle,setCardTitle,setIsCreated){
  try{
   const response= await axios.post(`https://api.trello.com/1/cards?idList=${listId}&key=${apiKey}&token=${apiToken}&name=${cardTitle}`);
...
const index = ({listId}) => {
  ...
  return (
    ...
    <Stack sx={{maxHeight:'50vh',overflowY:'auto'}} spacing={2} className='cards-container' onClick={(e)=>handleClick(e)}>
    {cards.map((card)=>(
            <Stack key={card.id} className='card' id={card.id} direction='row' spacing={2} padding={2} borderRadius={1} boxShadow={2} sx={{border:'1px solid grey',justifyContent:'space-between',backgroundColor:'white'}}>
            <Typography>{card.name}</Typography>
            <IconButton aria-label='card-delete-btn' className='card-delete-btn'><EditOutlinedIcon /></IconButton>
        </Stack>
           ))}
    </Stack>
     ...
     {/* for showing add card popup ------- */}
      ...
      {/* for showing delete card popup -------- */}
        <DeleteItem open={open} setOpen={setOpen} setIsClosed={setIsClosed} itemName='Card'/>
        ...
        <AlertMessage isCompleted={isClosed} setIsCompleted={setIsClosed} message={'SuccessFully Card archived !'}/>
    </>
  )
}
